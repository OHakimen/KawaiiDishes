...
public class AnimationsRegister {
    ...
    public static final Function<AnimationState<GenericGeoArmorItem>,PlayState> catTail = (event) -> {
        if((event.getExtraData().get(DataTickets.ENTITY) instanceof Player player && player.isCrouching())||(!(event.getExtraData().get(DataTickets.ENTITY) instanceof ArmorStand) && !(event.getExtraData().get(DataTickets.ENTITY) instanceof Player))){
