...
public class HastyAroma extends Aroma {
    public HastyAroma(TagKey<Item> items, int color) {
        super(items, color);
    }
    ...
    public void aromaTick(World pLevel, BlockPos pPos, BlockState pState, IncenseBlockEntity entity) {
        ...
        for (PlayerEntity player:entities) {
            player.setStatusEffect(new StatusEffectInstance(StatusEffects.HASTE, 15 * 20, 1, false ,false, false), player);
            player.setStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 15 * 20, 0, false ,false, false), player);
        }
    }
}
